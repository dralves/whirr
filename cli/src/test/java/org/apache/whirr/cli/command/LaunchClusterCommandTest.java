...
public class LaunchClusterCommandTest extends BaseCommandTest{
  ...
  public void testLaunchClusterUsingDryRun() throws Exception {
    ...
    int rc = launchCluster.run(null, out, err, ImmutableList.of("--cluster-name", "test-cluster-launch",
        "--state-store", "none", "--instance-templates", "1 zookeeper+cassandra, 1 zookeeper+elasticsearch",
        "--provider", "stub", "--identity", "dummy", "--private-key-file", keys.get("private").getAbsolutePath()));
